...
# Argument parsing
parser = argparse.ArgumentParser(description='FSRCNN test parameters')
...
Bicubic_image = image.resize((int(image_width), int(image_height)), Image.BICUBIC)  # upscale image using bicubic interpolation

bicubic_save_path = os.path.join(args.save_root, f"BICUBIC_{os.path.basename(args.image)}")
Bicubic_image.save(bicubic_save_path)
